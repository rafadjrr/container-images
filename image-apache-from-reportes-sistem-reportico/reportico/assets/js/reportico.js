...
function splitPage() {
      ...
      // Redondeamos al multiplo de 20 mas cercano (esto porque cada linea mide 20px)
      var pageheight = Math.round(reportico_jquery(this).innerHeight()/20.0) * 20.0;
        ...
        if ( nextlong > pageheight ) {
            ...
            if ( reportico_jquery(child).hasClass("reportico-page") ) {
                ...
                reportico_jquery(thispage).find("tr").each(function() {
                    if ( !looping ) 
                        return;
                    rheight = reportico_jquery(this).outerHeight(true);
                    rlong += rheight;
                    // Si la ultima linea se exede agregamos la clase overflow text y corta linea
                    if ( rlong >= pageheight) {
                        reportico_jquery(this).find("td").addClass("overflowtext");
                        if(rheight < 24){
...
** will generate full reportico output to replace the reportico-container tag
*/
reportico_jquery(document).on('click', '.reportico-admin-button, .csvOutputP, .reportico-admin-button2, .reportico-menu-item-link, .reportico-prepare-submit, .reportico-ajax-link, .reportico-ajax-link2, .reportico-link-menu2, .reportico-submit', function(event) 
{
    ...
    // Boton de CSV
    if(reportico_jquery(this).hasClass("csvOutputP")){
        var urlCsv = window.location.href;
        // Cambia la URL HTML por CSV
        var newUrlCsv = urlCsv.replace('HTML','CSV');
        window.open(newUrlCsv, '_blank');
        return;
    }
    ...
    if ( reportico_jquery(this).parent().hasClass("reportico-print-button" )  )
    {
        ...
        // Si el alto de la pagina es menor a 1000 es landscape y agregamos media query 
        if(reportico_jquery('.autopaginate').outerHeight() < 1000){
