...
def POST_no_return(diccionario):
  requests.request("POST", reqUrlPost, data=diccionario,  headers=headersList)

def GET_List():
  fecha = dt.datetime.today().date()
  reqUrlGetOne = "http://127.0.0.1:8000/multimoneda/tasas/?fecha__gte=%s" %fecha
  payload = {}
  response = requests.request("GET", reqUrlGetOne, data=payload,  headers=headersList)
  return response.text

def GET(existencia):
  reqUrlGetOne = "http://127.0.0.1:8000/multimoneda/tasas/?search=%s" %existencia
  payload = {}
  response = requests.request("GET", reqUrlGetOne, data=payload,  headers=headersList)
  return response.text

def buscaOne(idfuente,fuente,fecha,den,num):
    try:    
        conn = odbc.connect(connection_string(DRIVER_NAME,SERVER_NAME,DATABASE_NAME))
    except odbc.DatabaseError as e:
        print('Database Error:')
